...
SimpsonChars = {'Simpson4a': 'NedSimpson', 'Simpson4b': 'RodSimpson', 'Simpson5a': 'TodSimpson', 'Simpson5b': 'BazSimpson', 'LisaSimpsonITI': 'LisaSimpson'}

def RenameSimpsonsToMatch (origName, pattern):
    if 'Lisa' not in origName:
        newName = origName.replace(pattern,'')
        tName = newName.replace('Simpsons','Simpson')
        Num = str(re.findall('\d+', str(tName)))[2]
        loc = tName.split(str(Num))
        prefix = str(Num) + loc[1][0][0]
        newName = SimpsonChars[str('Simpson' + prefix)] + loc[1][1:]
        print newName
    else:
        tName = origName.split('ITI')
        newName = 'LisaSimpson' + tName[1]
        print newName
    return newName


for p in ['SimpsonsITI4a','SimpsonsITI4b','SimpsonsITI5a','SimpsonsITI5b','LisaSimpsonITI']:
    for c in data.filter(like=str(p)).columns.values:
        newName = RenameSimpsonsToMatch(c, 'ITI')
        data.rename(columns = {str(c):str(newName)}, inplace = True)

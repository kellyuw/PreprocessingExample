...
parser.add_argument('--subject', '-s', required=True, help='Name of subject')
args = parser.parse_args()

Subject = args.subject
Output = '/mnt/stressdevlab/dep_threat_pipeline/' + Subject + '/'

#Check values of ProcedureBlock column
#Blocks = pd.unique(data.ProcedureBlock.ravel())
#print(Blocks)

#This function matches task names from behavioral file to name structure in dir.
def GetAlias (Task):
    #SplitStr = ProcName.rpartition('Run')

    Aliases = {'ExtinctionRecall':['extrecall','ExtRecall'], 'ThreatReactivity':['facereactivity','FaceReactivity'],
                'WMShapes':['wmshape','WMShape'], 'WMFaces':['wmface','WMFace'], 'GNG':['gng','GNG']}
    return(Aliases[Task])


def RemoveSimpsons (data, Task, alias):
    ...
    for OnsetTime in data.filter(like="SimpsonOnsetTime").columns.values:
        ...
        OutputFile=open((Output + '/' + alias[0] + '/' + alias[1] + RunNum + '_Simpson.txt'), "w")

...
CurrentProc='ThreatReactivity'+str(Run)
...
for Condition in ['C','S','F']:
	...
	BlockOnsets=(data['ReactivityITIOnsetTime'][np.logical_and(np.logical_and(data['EmotionSubTrial']==Condition,data['ProcedureBlock']==('ThreatReactivity'+str(Run))),data['SubTrial']==1)]-data['TriggerWAITRTTimeBlock'][np.logical_and(np.logical_and(data['EmotionSubTrial']==Condition,data['ProcedureBlock']==('ThreatReactivity'+str(Run))),data['SubTrial']==1)])/float(1000)
	BlockOffsets=(data['ReactivityStimulusOffsetTime'][np.logical_and(np.logical_and(data['EmotionSubTrial']==Condition,data['ProcedureBlock']==('ThreatReactivity'+str(Run))),data['SubTrial']==36)]-data['TriggerWAITRTTimeBlock'][np.logical_and(np.logical_and(data['EmotionSubTrial']==Condition,data['ProcedureBlock']==('ThreatReactivity'+str(Run))),data['SubTrial']==36)])/float(1000)
